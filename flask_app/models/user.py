...
class User:
    ...
    def validate_user(user):
        ...
        if len(user['first_name']) <3:
            flash('First name must be at lest 3 characters', 'firstNameRegister')
            is_valid= False
        if len(user['last_name']) <3:
            flash('Last name must be at lest 3 characters', 'lastNameRegister')
            is_valid= False
        if not EMAIL_REGEX.match(user['email']): 
            flash("Invalid email address!", 'emailRegister')
            is_valid = False
        if len(user['password']) <8:
            flash('Password must be at lest 8 characters !', 'passwordRegister')
            is_valid= False
